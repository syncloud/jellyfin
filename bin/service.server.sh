...
exec ${DIR}/lib/*-linux*/ld-*.so --library-path $LIBS $DIR/app/jellyfin/jellyfin --configdir /var/snap/jellyfin/current/config --datadir /var/snap/jellyfin/current/data --cachedir /var/snap/jellyfin/current/cache --ffmpeg /snap/jellyfin/current/bin/ffmpeg.sh
